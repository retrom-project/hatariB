...
work=$(mktemp -d "$root/.retrom-build/retrom-hatarib-web.XXXXXX")
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-hatarib \
  ...
  /recipe/build-emulatorjs-core.sh hatarib \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/hatarib_libretro.js" "$stage/"
install -m 0644 "$work/raw/hatarib_libretro.wasm" "$stage/"
cat "$root/LICENSE" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.2.2","version":"1.18"}' > "$stage/build.json"
printf '%s\n' '{"name":"hatarib","extensions":["st","msa","dim","stx","ipf"],"makeoptions":{"buildpath":"./","makescript":"makefile.libretro","arguments":[]},"options":{},"save":true,"license":"LICENSE","repo":"https://github.com/retrom-project/hatariB"}' > "$stage/core.json"
chmod 0644 "$stage/build.json" "$stage/core.json" "$stage/license.txt"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/hatarib-wasm.data" \
  hatarib_libretro.js hatarib_libretro.wasm build.json core.json license.txt)
